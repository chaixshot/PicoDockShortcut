...
*   **🎯 自定义 Dock 应用列表：** 拦截并重写 `dock_fix_apps.json`，固定您想要的应用。
*   **🧩 运动中心控制：** 通过 GUI 开关控制硬编码的“运动中心” Dock 条目。
*   **🖼️ Dock 背景：** 为 Dock 栏设置自定义图片，支持固定比例裁剪。
...
## 🎯 自定义 Dock 应用列表
应用通过拦截 `com.pvr.shortcut` 的 `dock_fix_apps.json`，让您可以通过 GUI 配置固定的 Dock 应用，而无需手动编辑文件。

## 🧩 运动中心控制
“运动中心”是中文/凤凰版固件中硬编码的 Dock 条目。此版本将其转换为 GUI 开关，以便可以显式删除或保留。

## 🖼️ 自定义 Dock 背景
您可以为 Dock 栏设置自定义图片，内置固定比例裁剪功能，并为新手引导栏提供共享背景。
